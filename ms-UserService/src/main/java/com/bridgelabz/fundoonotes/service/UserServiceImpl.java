...
//				return user;
		...
		@Override
		public UserDetails verifyEmail(String token, String emailId, HttpServletRequest request) {
			UserDetails user = userDetailsRepository.findById(generateToken.verifyToken(token)).get();
			List<UserDetails> users = userDetailsRepository.findAllByEmailId(emailId);
			if (users.size() == 1) {
